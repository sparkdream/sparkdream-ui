/**
 * Module parameter metadata shared by the governance parameter-change form
 * (which writes params) and the proposal diff view (which reads them back off
 * a submitted `MsgUpdateParams`). Keeping the field table in one place means a
 * newly-described param is labelled and formatted identically in both.
 */
 ...
 */
export type GenericLoader = () => Promise<{
...
export function displayValue(field: FieldDef, raw: unknown): string {
...
/** Find the module whose `MsgUpdateParams` a proposal message carries. The
 * LCD renders the type URL with a leading slash ("/sparkdream.rep.v1.Msg…"),
 * the same form we store, but tolerate its absence. */
export function findModuleByTypeUrl(
  typeUrl: string
): { key: string; module: ModuleDef } | null {
  const want = typeUrl.startsWith("/") ? typeUrl : `/${typeUrl}`;
  for (const [key, module] of Object.entries(MODULES)) {
    if (module.typeUrl === want) return { key, module };
  }
  return null;
}

/** Field definition for an LCD param path (the dot-path form used by
 * `apiKey`), so a diff can borrow the form's label, kind and unit. */
export function findFieldByApiKey(
  module: ModuleDef,
  apiKey: string
): FieldDef | undefined {
  return module.fields.find((f) => f.apiKey === apiKey);
}
